 ...
 * baseUrl编码
 */
export const base64UrlEncode = (source: string): string => {
  return btoa(source)
    .replace(/=+$/, '')
    .replace(/\+/g, '-')
    .replace(/\//g, '_');
 ...
 * HS256
 */
export const HMACSHA256 = (source: string, key?: string) => {
  if (key === undefined) {
    key = '';
  }
  let hmac = createHmac('sha256', key);
  return hmac.update(source).digest('base64url');
};

/**
 * MD5
 */
export const MD5 = (source: string): string => {
  ...
  function md5_ConvertToWordArray(string: string): number[] {
    let lWordCount;
  ...
  let S41 = 6,
    ...
    S43 = 15,
    S44 = 21;
  source = md5_Utf8Encode(source);
  x = md5_ConvertToWordArray(source);
  a = 0x67452301;
  b = 0xefcdab89;
  c = 0x98badcfe;
  d = 0x10325476;
